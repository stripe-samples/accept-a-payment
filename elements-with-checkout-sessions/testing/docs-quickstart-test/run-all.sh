...
for idx in "${!LANGUAGES[@]}"; do
  ...
  if [[ ",$SKIP_LANGS," == *",$lang,"* ]]; then
    warn "Skipping $lang (--skip)"
    continue
  fi
  if ! $REACT_ONLY; then
    start_html_server "$lang" "$idx" || warn "Failed to start $lang-html, continuing..."
  fi
  if ! $HTML_ONLY; then
    start_react_server "$lang" "$idx" || warn "Failed to start $lang-react, continuing..."
  fi
done
